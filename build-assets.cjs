...
function build(root=__dirname){
 ...
 for(const folder of ['icons','textures']){fs.mkdirSync(path.join(dir,folder),{recursive:true});scan(folder);}
 const records=manifest.map(a=>{const absolute=path.resolve(dir,a.file),rel=path.relative(dir,absolute);if(rel.startsWith('..')||path.isAbsolute(rel)||!['icons','textures'].includes(a.folder))throw Error('Invalid asset path: '+a.file);
