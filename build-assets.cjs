...
const records=manifest.map(a=>({...a,sha256:require('crypto').createHash('sha256').update(fs.readFileSync(path.join(dir,a.file))).digest('hex'),src:'data:image/png;base64,'+fs.readFileSync(path.join(dir,a.file)).toString('base64')}));
...
fs.writeFileSync(path.join(dir,'catalog-3.js'),'/* Additional original assets; see PACK-03.md. */\nwindow.FLYRA_ASSETS.push(...'+JSON.stringify(records.slice(12))+');\n');
